...
export const NewsDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([art, all]) => {
        setArticle(art);
        setOtherNews(all.filter((n) => n.slug !== slug).slice(0, 3));
      })
      .catch(() => setError(true))
  ...
  if (error || !article) {
    return (
      <Box pt="calc(64px + 5rem)" pb={20} textAlign="center">
        <Container maxW="6xl">
          <Heading fontWeight={400} fontSize="2xl" mb={4}>
            Nyheten hittades inte
          </Heading>
          <Text color="brand.muted" mb={8}>
            Kontrollera att länken är korrekt.
          </Text>
          <Button as={RouterLink} to="/nyheter" variant="brandDark">
    ...
    <Box as="article" pt="calc(64px + 3rem)" pb={{ base: 16, md: 24 }} bg="brand.bg">
      <Container maxW="6xl">
        <Breadcrumb fontSize="sm" color="brand.muted" mb={10} separator="/">
          <BreadcrumbItem>
            <BreadcrumbLink as={RouterLink} to="/" _hover={{ color: "brand.olive" }}>
              Hem
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem>
            <BreadcrumbLink as={RouterLink} to="/nyheter" _hover={{ color: "brand.olive" }}>
              Nyheter
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem isCurrentPage>
            <BreadcrumbLink color="brand.text">{article.title}</BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
        ...
        <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={{ base: 10, lg: 16 }} templateColumns={{ lg: "1fr 300px" }} alignItems="start">
          <Reveal>
            ...
            <VStack align="stretch" spacing={6}>
              {article.content.map((p, i) => (
                <Text key={i} fontSize={{ base: "md", md: "lg" }} lineHeight={1.85} color="brand.text">
                  {p}
                </Text>
              ))}
            </VStack>
          ...
          <Reveal direction="right">
            <Box
              ...
              top={{ lg: "6rem" }}
            >
              <Heading fontWeight={400} fontSize="lg" mb={5}>
                Fler nyheter
              </Heading>
                  ...
                  {otherNews.map((n) => (
                    <ListItem key={n.id}>
                      <ChakraLink
                        ...
                        to={`/nyheter/${n.slug}`}
                        display="block"
                        _hover={{ color: "brand.olive", textDecoration: "none" }}
                      ...
                      >
                        ...
                        <Text fontSize="xs" color="brand.muted">
                          {new Date(n.published_at).toLocaleDateString("sv-SE")}
                        </Text>
                      </ChakraLink>
                    </ListItem>
                  ))}
